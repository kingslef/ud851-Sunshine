...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadWeatherData(String preferred_location) {
        URL url = NetworkUtils.buildUrl(preferred_location);
        if (url == null) {
            return;
        }

        new NetworkRequester().execute(url);
    }

    // COMPLETED (5) Create a class that extends AsyncTask to perform network requests
    private class NetworkRequester extends AsyncTask<URL, Void, String[]> {
        ...
        protected String[] doInBackground(URL... urls) {
            ...
            try {
                weatherData = OpenWeatherJsonUtils.getSimpleWeatherStringsFromJson(MainActivity.this, jsonResult);
